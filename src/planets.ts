...
export function groundRadius(p: Planet, dir: THREE.Vector3): number {
  if (p.voxelGrid > 0) {
    ...
    return p.radius + Math.round(h / VOXEL_BLOCK) * VOXEL_BLOCK
  }
...
/**
 * A loose ring of four, tilted off the horizontal, plus the lagoon sitting
 * above the ring. Surface-to-surface gaps between neighbours run 8-16 units:
 * one held jump crosses them.
 *
...
export const PLANETS: Planet[] = [
  planet('home', [21, 0, 0], 8, {
    land: 0x8fd8a4,
    ocean: 0x46a8d8,
    seaLevel: 0.545,
    seed: 1207,
  }),
  planet('coral', [0, 4, 21], 5, {
    land: 0xffc79c,
    ocean: 0xd8756c,
    seaLevel: 0.56,
    seed: 3391,
  ...
  // Set well apart from the rest, out past the lilac planet — that is the one
  // world close enough to jump from, so reaching the meadow means crossing the
  ...
  planet(
    ...
    9,
    {
      ...
      island: { dir: [0.42, 0.28, 0.86], extent: 0.85 },
    },
    { relief: 1.6, noise: 2.0 },
  ),
